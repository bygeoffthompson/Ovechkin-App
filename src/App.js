...
function SearchForm({jsonData}) {
    ...
    const slick = {
        ...
        afterChange: function(index) {
            let currentFrame = document.querySelector('.slick-current iframe')
            if (!currentFrame.hasAttribute('src')) {
                let currentSrc = currentFrame.getAttribute('data-src')
                currentFrame.setAttribute('src', currentSrc)
            }
        }
    };
    ...
    return (
        <div>
            ...
            <Slider key={searchResults.id || searchResults.length} {...slick}>
                {searchResults.map((result, index) => (
                    <div className="frame" data-jersey={result.jersey} data-season={result.season} key={index}>
                        ...
                        <div className="shadow">
                            <iframe width="560" height="315" {...(index === 0 ? { 'src': 'https://www.youtube.com/embed' + result.link.replace(/"/g, "") + '&autohide=0&rel=0&modestbranding=1' } : {})} {...(index !== 0 ? { 'data-src': 'https://www.youtube.com/embed' + result.link.replace(/"/g, "") + '&autohide=0&rel=0&modestbranding=1' } : {})} title="Alex Ovechkin Goal Video" referrerPolicy="cross-origin-with-strict-origin" allowFullScreen></iframe>
                        </div>
                        <div className="tags">
